...
#train agent with reward packages
    def train(self, num_episodes):
        for episode in range(num_episodes):
            self.newEpoch()
            correctOrder = [FourRooms.RED, FourRooms.GREEN, FourRooms.BLUE]
            storeCollectedPackages = []
            while not self.isTerminal():
                ...
                if gridType > 0:
                    #store the packages collected
                    storeCollectedPackages.append(gridType)
                    #compare the packages collected against the correct deqence 
                if storeCollectedPackages != correctOrder[:len(storeCollectedPackages)]:
                    break
